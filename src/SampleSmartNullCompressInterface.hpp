...
class SampleSmartNullCompressInterface : public CompressInterface {
  public:
    enum CompressScheme { BITPACKING_C, RLE_C, DELTA_C, NUM_C_SCHEMES };
    /**
     * Construct a SampleCompressInterface object
     */
    SampleSmartNullCompressInterface(const std::string &codec_name = "");

    ~SampleSmartNullCompressInterface() override;
  ...
  private:
    ...
    CompressScheme sampleStat(const uint32_t *src_arr,
                              const std::vector<bool> &src_null, uint32_t n);

    struct SampleStat {
        uint32_t num_elements, num_runs;
        uint32_t last, delta_last;
        ...
        CompressScheme compute() {
            ...
            const double delta_bytes =
                avg_delta_bits / 8 * num_elements + bp_stat.block_cnt;
            return (bp_bytes <= rle_bytes)
                       ? ((bp_bytes <= delta_bytes) ? BITPACKING_C : DELTA_C)
                       : ((rle_bytes <= delta_bytes) ? RLE_C : DELTA_C);
        }
        void incData(uint32_t cur, bool isNull) {
            uint32_t delta =
                encodeZigZagValInt32(int32_t(cur) - int32_t(delta_last));
            delta_last = cur;
            (isNull) ? (cur = last) : (0);
            bp_stat.incData(cur, delta);
            num_runs += (cur != last), last = cur;
        }
        void setLast(uint32_t last_v) {
            last = last_v;
            delta_last = last_v;
        }
    };
    std::vector<uint32_t> fillNullsLast(std::string_view src,
                                        const std::vector<bool> &nulls);
    std::vector<uint32_t> fillNullsMostFreq(std::string_view src,
                                            const std::vector<bool> &nulls);
    std::vector<uint32_t> fillNullsRandom(std::string_view src,
                                          const std::vector<bool> &nulls);
